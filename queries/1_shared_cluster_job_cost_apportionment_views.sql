...
CREATE OR REPLACE VIEW finops.system_lookups.v_shared_cluster_creator_job_duration AS 
...
CREATE OR REPLACE VIEW finops.system_lookups.v_shared_cluster_job_duration_cost AS 
    SELECT 
    w.workspace_id, 
    w.workspace_name,
    jrt.cluster_id, 
...
FROM finops.system_lookups.v_shared_cluster_creator_job_duration job_duration
...
CREATE OR REPLACE VIEW  finops.system_lookups.v_shared_cluster_creator_job_duration_weighted_cost AS 
...
FROM finops.system_lookups.v_shared_cluster_creator_job_duration creator_duration
INNER JOIN finops.system_lookups.v_shared_cluster_job_duration_cost AS cluster_cost_duration
...
-- final apportionment
CREATE OR REPLACE VIEW finops.system_lookups.v_cost_byshared_cluster_daily_apportionment AS 
...
FROM finops.system_lookups.v_shared_cluster_creator_job_duration_weighted_cost creator_weighted_cost
-- cartesian
INNER JOIN (
  SELECT sum(exec_duration_weighted_cluster_cost) as total_exec_duration_weighted_cluster_cost, usage_date
  FROM finops.system_lookups.v_shared_cluster_creator_job_duration_weighted_cost
  GROUP BY usage_date
...
CREATE OR REPLACE VIEW  finops.system_lookups.v_cost_bytag_daily_apportionment AS 
